...
if __name__ == '__main__':
    ...
    common_lib.check_if820_response(
        if820_central.EVENT_SMP_PASSKEY_DISPLAY_REQUESTED, response)
    conn_handle = response[1]

    # IF820 Event (Text Info contains "P")
    app_logger.app_log_info("Wait for IF820 Pairing Requested Event.")
    response = if820_central.wait_event(
        event=if820_central.EVENT_SMP_PAIRING_REQUESTED)
    common_lib.check_if820_response(
        if820_central.EVENT_SMP_PAIRING_REQUESTED, response)
    ...
    response = if820_central.wait_event(
        event=if820_central.EVENT_SMP_PAIRING_RESULT)
    common_lib.check_if820_response(
        if820_central.EVENT_SMP_PAIRING_RESULT, response)
    ...
    response = if820_central.wait_event(
        event=if820_central.EVENT_SMP_ENCRYPTION_STATUS)
    common_lib.check_if820_response(
        if820_central.EVENT_SMP_PASSKEY_DISPLAY_REQUESTED, response)
